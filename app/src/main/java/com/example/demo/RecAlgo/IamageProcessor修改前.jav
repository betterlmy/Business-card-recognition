...
public class ImageProcessor {
    ...
    public static Mat preProc(@NonNull Mat rotated) {
        ...
        double scale=rotated.width()/1670.0;
        namecarddefaultSize=new Size(rotated.width()/scale,rotated.height()/scale);
        resize(rotated, rotated, namecarddefaultSize, 0, 0, INTER_AREA);//调整大小
        bm= showBm(rotated);
//        show(gray,imageView,context,"灰度");

        //图像二值化
        Mat binary = new Mat();
//        threshold(rotated, binary, 0, 255, THRESH_BINARY | THRESH_OTSU);
        threshold(rotated, binary, 200, 255, THRESH_BINARY_INV);
        bm= showBm(binary);


        //降噪处理
//        medianBlur(binary, binary, 5);
//        show(binary,imageView,context,"降噪");


        //轮廓发现
        List<MatOfPoint> contours = new ArrayList<MatOfPoint>();
        Mat hierarchy = new Mat();
        findContours(binary, contours, hierarchy, RETR_TREE, CHAIN_APPROX_SIMPLE, new Point(0, 0));
        ...
        if (rect.width > 0)
            namecardMat = new Mat(binary, rect1);
        scale=lr.get(secNum).width/namecarddefaultSize.width;
